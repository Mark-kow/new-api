...
func processTokens(relayMode int, streamItems []string, responseTextBuilder *strings.Builder, toolCount *int) error {
	// Performance optimization: Calculate exact byte capacity needed to avoid intermediate string allocations
	// like strings.Join and string concatenation. json.Unmarshal natively processes []byte anyway.
	capacity := 2 // '[' and ']'
	if len(streamItems) > 0 {
		capacity += len(streamItems) - 1 // commas
