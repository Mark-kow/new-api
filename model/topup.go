...
func RechargeDirect(tradeNo string, paymentMethod string, providerTradeNo string, providerPayload string) (err error) {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	var quotaToAdd int
	...
	if err != nil {
		common.SysError("direct topup failed: " + err.Error())
		return errors.New("充值失败，请稍后重试")
	}

	if quotaToAdd > 0 {
		RecordLog(topUp.UserId, LogTypeTopup, fmt.Sprintf("官方支付充值成功，支付方式: %s，充值额度: %v，支付金额: %.2f", paymentMethod, logger.FormatQuota(quotaToAdd), topUp.Money))
	}
